...
public class HallFunctions extends TCPGameFunctions {
	...
	public synchronized static void interface_100007(WSClient channel, Map<String, Object> readData) {
		...
		if (roomType == null) {
			...
			// 设置用户信息
			p.setPosition(getWind(userIds,userId));// 设置庄家位置
...
//			userIds[p.getPosition() - 1] = p.getUserId();//在getWind里设置了
	...
	public synchronized static void interface_100008(WSClient channel, Map<String, Object> readData) {
		...
		p.setRoomId(roomId);
		p.setPosition(getWind(userIds,userId));
...
//		userIds[p.getPosition() - 1] = p.getUserId();//在getWind里设置了
		...
		info.put("playType", room.getPlayType());
		info.put("playerNum", room.getPlayerNum());
		info.put("sanQiSuanJia", room.getSanQiSuanJia());
		info.put("danDiaoSuanJia", room.getDanDiaoSuanJia());
		info.put("gangScore", room.getGangScore());
		info.put("scoreType", room.getScoreType());
		info.put("qiangGangHu", room.getQiangGangHu());
		if(room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)){
			info.put("hongZhongBao", room.getHongZhongBao());
			info.put("guaDaFeng", room.getGuaDaFeng());
			info.put("buJiaBuHu", room.getBuJiaBuHu());
		}
		...
		for(Long ids:userIds){
			...
			if (ids.equals(userId)) {
				...
				getJoinRoomUserInfo(p, userInfo,2);
		...
		if (hgetAll != null && hgetAll.size() > 0) {
			...
			for (String roomId : keySet) {
				...
				if (room != null && room.getCreateId().equals(userId) && room.getState() != Cnst.ROOM_STATE_YJS && room.getRoomType() == Cnst.ROOM_TYPE_2) {
					...
					map.put("playType", room.getPlayType());
					map.put("playerNum", room.getPlayerNum());
					map.put("sanQiSuanJia", room.getSanQiSuanJia());
					map.put("danDiaoSuanJia", room.getDanDiaoSuanJia());
					map.put("gangScore", room.getGangScore());
					map.put("scoreType", room.getScoreType());
					map.put("qiangGangHu", room.getQiangGangHu());
					if(room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)){
						map.put("hongZhongBao", room.getHongZhongBao());
						map.put("guaDaFeng", room.getGuaDaFeng());
						map.put("buJiaBuHu", room.getBuJiaBuHu());
					}
...
//			}
	 ...
	 * @return
	 */
	public static Integer getWind(Long[] userIds,Long userId) {
		...
		int position  = ps.get(CommonUtil.getRamdonInNum(ps.size()));
		
		if (userIds.length==2) {
			if (position==Cnst.WIND_WEST) {
				userIds[position-2] = userId;
			}else{
				userIds[position-1] = userId;
			}
		}else{
			userIds[position-1] = userId;
		}
