...
public class MessageFunctions extends TCPGameFunctions {
	...
	public static Map<String, Object> getCurrentUserInfo(Player player, RoomResp room) {
		...
		currentUserInfo.put("ting", player.getTing());
	...
	public static Map<String, Object> getRoomInfo(RoomResp room,Player p) {
		...
		roomInfo.put("playerNum", room.getPlayerNum());
		roomInfo.put("sanQiSuanJia", room.getSanQiSuanJia());
		roomInfo.put("danDiaoSuanJia", room.getDanDiaoSuanJia());
		roomInfo.put("gangScore", room.getGangScore());
		roomInfo.put("scoreType", room.getScoreType());
		roomInfo.put("qiangGangHu", room.getQiangGangHu());
		if(room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)){
			roomInfo.put("hongZhongBao", room.getHongZhongBao());
			roomInfo.put("guaDaFeng", room.getGuaDaFeng());
			roomInfo.put("buJiaBuHu", room.getBuJiaBuHu());
		}
		//修改为暗宝了
//		if (room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)&&p.getTing()!=null) {
//			roomInfo.put("baoPai", room.getBaoPai());//如果玩家听牌，才会给玩家宝牌信息
//		}
		if (p.getTing()!=null&&p.getTing().equals(Cnst.TING_STATE_1)) {
			roomInfo.put("tingList", p.getTingToChuList());
		}
	...
	public static void interface_100104(RoomResp room, List<Player> players,Integer interfaceId,Action action) {
		boolean hasHuanBao = false;
		boolean hasTingAction = false;
		try {  
			Integer actId= action.getActionId();
			if (actId!=null&&((actId >=175 && actId <=196)||(actId >=197 && actId <=230)||action.getActionId().equals(Cnst.ACTION_BIANMA_TING))) {
				hasTingAction = true;
				hasHuanBao = checkHuanBao(players,room);
			}else if(actId!=500){//不是胡的时候
				if (hasTing(players)) {//如果有人听， 每次行为除了胡，都要计算是否换宝
					hasHuanBao = checkHuanBao(players,room);
				}
			}
		} catch (Exception e) {
		...
		for (Player p : players) {
			...
			if (hasTingAction&&p.getUserId().equals(action.getUserId())) {
//				info.put("baoPai", room.getBaoPai());
				info.put("currMJNum", room.getCurrentMjList().size());
				info.put("tingList", p.getTingToChuList());
			}
			if (hasHuanBao) {
//				if (p.getTing()!=null) {
//					info.put("baoPai", room.getBaoPai());
//				}
	...
	private static boolean hasTing(List<Player> players){
		boolean hasTing = false;
		for (int i = 0; i < players.size(); i++) {
			if (players.get(i).getTing()!=null) {
				return true;
			}
		}
		return hasTing;
	...
	/**
	 * 
	 * @Title: toCheckHuanBao   
	 * @Description: 换宝的逻辑
	 * @param: @return      
	 * @return: boolean      
	 * @throws
	 */
	private static boolean checkHuanBao(List<Player> player,RoomResp room){
		boolean hasHuanBao = false;
		List<Integer> allZhuoMianShangLiangDePais = new ArrayList<Integer>();//所有桌面上亮的牌，在开局发牌的时候生成了一张宝牌，所以，换宝的时候，就不用考虑玩家的杠牌了，只用把吃、碰加到里面检测就行
		for (int i = 0; i < player.size(); i++) {
			//先添加玩家吃碰的牌
			List<Action> acts = player.get(i).getActionList();
			if(acts!=null&&acts.size()>0){
				for (int j = 0; j < acts.size(); j++) {
					Action act = acts.get(j);
					if (act.getActionId()>=35&&act.getActionId()<=56) {
						int[] chis = Cnst.chiMap.get(act.getActionId());
						for (int m = 0; m < chis.length; m++) {
							allZhuoMianShangLiangDePais.add(chis[m]);
						}
					}else if(act.getActionId() >=175 && act.getActionId() <=196){
						int[] chis = Cnst.chiMap.get(act.getActionId()-140);
						for (int m = 0; m < chis.length; m++) {
							allZhuoMianShangLiangDePais.add(chis[m]);
						}
					}else if(act.getActionId()>=57&&act.getActionId()<=90){
						allZhuoMianShangLiangDePais.add(act.getActionId()-56);
						allZhuoMianShangLiangDePais.add(act.getActionId()-56);
						allZhuoMianShangLiangDePais.add(act.getActionId()-56);
					}else if(act.getActionId() >=197 && act.getActionId() <=230){
						allZhuoMianShangLiangDePais.add(act.getActionId()-56-140);
						allZhuoMianShangLiangDePais.add(act.getActionId()-56-140);
						allZhuoMianShangLiangDePais.add(act.getActionId()-56-140);
					}
				}
			...
			//再添加玩家出的牌
			allZhuoMianShangLiangDePais.addAll(player.get(i).getChuList());
		}
		//拿到当前局的宝牌
		Integer currBao = room.getBaoPai();
		int baoNum = 0;
		if (allZhuoMianShangLiangDePais.size()>0) {
			baoNum = Collections.frequency(allZhuoMianShangLiangDePais, currBao);//本方法可以统计后边的参数在前面的参数里出现的次数
		}
		if (baoNum>=3) {//理论上很定是小于等于3的
			hasHuanBao = true;
			while(room.getCurrentMjList().size()>12){
				int pai = MahjongUtils.faPai(room.getCurrentMjList(), 1).get(0);
				if(Collections.frequency(allZhuoMianShangLiangDePais, pai)<3){
					room.setBaoPai(pai);
					break;
				}
			}
	...
	//测试Collections.frequency效率
	public static void test(String[] args) {
		List<Integer> list = new ArrayList<Integer>();
		for (int i = 0; i < 1000000; i++) {
			list.add(CommonUtil.getRamdonInNum(2000));
		}
		System.out.println("数字\t\t次数\t\t挨个统计的次数");
		for (int i = 0; i < list.size(); i++) {
			int n = list.get(i);
			long l1 = System.currentTimeMillis();
			int num = Collections.frequency(list, n);
			...
			System.out.println(num1+"\t\t"+(num==num1)+"\t\t"+(l2-l1)+"\t\t"+(l4-l3));
		}
	}
