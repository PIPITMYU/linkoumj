...
public class GameFunctions extends TCPGameFunctions {
	...
	public static void startGame(RoomResp room, List<Player> players) {
		...
		if (room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)) {//带宝玩儿法
			//定宝牌,开局的时候随便定一个就行
			room.setBaoPai(MahjongUtils.faPai(room.getCurrentMjList(), 1).get(0));
		}
		
		//3.1看庄家有没有暗杠.带混检测, 有没有胡牌等.
		List<Integer> actionList = MahjongUtils.checkActionList(zhuangPlayer,zhuangPlayer.getCurrentMjList().get(13),room,Cnst.CHECK_TYPE_ZIJIMO,false,players);
	...
	public synchronized static void interface_100201(WSClient channel, Map<String, Object> readData) {
		...
		in:if(action == -1){//发牌
			...
			if(room.getCurrentMjList().size() < 12){ 	
				...
				List<Integer> currentPlayerActionList = MahjongUtils.checkActionList(currentPlayer, haiDiPai, room,Cnst.CHECK_TYPE_HAIDIANPAI,false,players);
				...
				List<Integer> currentPlayerActionList = MahjongUtils.checkActionList(currentPlayer, pai, room,Cnst.CHECK_TYPE_ZIJIMO,false,players);
		...
		//过	
			...
			if(room.getIsQiangGangHu()!=null && room.getIsQiangGangHu() == true){
				...
				room.getQiangGangHuList().remove(userId);
				if(room.getQiangGangHuList().size() == 0){
					...
					nextAction.add(0);
					room.setNextAction(nextAction);
					room.setNextActionUserId(room.getQiangGangHuList().get(0));
				}
				...
				if(room.getLastFaPaiUserId().equals(userId) && room.getGuoUserIds().size() == 0){
					...
					//如果上一个动作是自己，而且是吃或者碰，过了之后，该自己出牌
					if (room.getLastActionUserId()!=null&&room.getLastActionUserId().equals(userId)&&room.getLastAction()!=null&&room.getLastAction()>=35&&room.getLastAction()<=90) {
			...
			if(room.getIsQiangGangHu() != null && room.getIsQiangGangHu() == true){
				...
				for(Player p:players){
					if(p.getUserId().equals(room.getLastActionUserId())){
						p.setIsDian(true);
//						p.setGangNum(p.getGangNum() == null ? 0 : p.getGangNum()-1);//没有统计杠的次数
		...
		//吃	
		}else if((action >=35 && action <=56)||(action >=175 && action <=196)){ //吃
			...
			MahjongUtils.checkTingToChuList(currentPlayer, room,players);
			if (action >=35 && action <=56&&currentPlayer.getTingToChuList()!=null) {
		...
		//碰
			...
			//碰完是请求出牌
			List<Integer> actionList = new ArrayList<Integer>();
			if (action >=57 && action <=90&&currentPlayer.getTingToChuList()!=null) {
				actionList.add(Cnst.ACTION_BIANMA_TING);
				actionList.add(0);
			}else{
				actionList.add(501);
		...
		//杠	
			...
			if(gangPaiNum == 1){
				...
				if (room.getQiangGangHu().equals(Cnst.YES)) {
					for(Player p:players){
						if(p.getUserId().equals(userId)){
							continue;
						}
						List<Integer> qiangGang = MahjongUtils.checkActionList(p, pai, room,Cnst.CHECK_TYPE_QIANGGANG,false,players);
						if(qiangGang.size() > 0){
							room.getQiangGangHuList().add(p.getUserId());
