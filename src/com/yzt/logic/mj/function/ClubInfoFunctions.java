...
public class ClubInfoFunctions extends TCPGameFunctions {
	...
	public static void interface_500001(WSClient channel, Map<String, Object> readData) throws Exception {
		...
		if (null != redisClub) {
			...
			info.put("clubUserName", redisClub.getClubUserName());
...
//					Player roomCreater = RedisUtil.getPlayerByUserId(String.valueOf(room.getCreateId()));
					...
					jsobj.put("playType", String.valueOf(room.getPlayType()));
		    		jsobj.put("playerNum", String.valueOf(room.getPlayerNum()));
		    		jsobj.put("sanQiSuanJia", String.valueOf(room.getSanQiSuanJia()));
		    		jsobj.put("danDiaoSuanJia",String.valueOf(room.getDanDiaoSuanJia()));
		    		jsobj.put("gangScore", String.valueOf(room.getGangScore()));
		    		jsobj.put("scoreType", String.valueOf(room.getScoreType()));
		    		jsobj.put("qiangGangHu", room.getQiangGangHu());
		    		if(room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)){
		    			jsobj.put("hongZhongBao", String.valueOf(room.getHongZhongBao()));
		    			jsobj.put("guaDaFeng", String.valueOf(room.getGuaDaFeng()));
		    			jsobj.put("buJiaBuHu", String.valueOf(room.getBuJiaBuHu()));
		    		}
...
//			}
	...
	public synchronized static void interface_500004(WSClient channel, Map<String, Object> readData) {
		...
		Integer clubId = StringUtils.parseInt(readData.get("clubId"));// 俱乐部id
		
		Integer playType = StringUtils.parseInt(readData.get("playType"));// 玩法
		Integer playerNum = StringUtils.parseInt(readData.get("playerNum"));//人数
		Integer sanQiSuanJia = StringUtils.parseInt(readData.get("sanQiSuanJia"));//三七算夹
		Integer danDiaoSuanJia = StringUtils.parseInt(readData.get("danDiaoSuanJia"));//单吊算夹
		Integer gangScore = StringUtils.parseInt(readData.get("gangScore"));//明杠分
		Integer scoreType = StringUtils.parseInt(readData.get("scoreType"));//点炮包法
		Integer qiangGangHu = StringUtils.parseInt(readData.get("qiangGangHu"));//抢杠胡
		
		RoomResp room = new RoomResp();
		room.setPlayType(playType);
		room.setQiangGangHu(qiangGangHu);
		if(playType!=null&&playType.equals(Cnst.PLAY_TYPE_DAIBAO)){
			Integer hongZhongBao = StringUtils.parseInt(readData.get("hongZhongBao"));//红中宝
			Integer guaDaFeng = StringUtils.parseInt(readData.get("guaDaFeng"));//刮大风
			Integer buJiaBuHu = StringUtils.parseInt(readData.get("buJiaBuHu"));//不夹不胡
			room.setHongZhongBao(hongZhongBao);
			room.setGuaDaFeng(guaDaFeng);
			room.setBuJiaBuHu(buJiaBuHu);
		}else{
			room.setHongZhongBao(Cnst.NO);
			room.setGuaDaFeng(Cnst.NO);
			room.setBuJiaBuHu(Cnst.NO);
		}
		room.setPlayerNum(playerNum);
		room.setSanQiSuanJia(sanQiSuanJia);
		room.setDanDiaoSuanJia(danDiaoSuanJia);
		room.setGangScore(gangScore);
		
		Player p = RedisUtil.getPlayerByUserId(String.valueOf(userId));
		Integer needMoney = Cnst.getClubRoomNeedMoney(playerNum, circleNum);
		Map<String, String> roomMap = RedisUtil.hgetAll(Cnst.REDIS_CLUB_ROOM_LIST.concat(String.valueOf(clubId)));
		if (roomMap.isEmpty()) {
			// 似乎也不用做处理
		} else {
		...
		if ((todayUse + needMoney / room.getPlayerNum()) > max) {
			JSONObject error = new JSONObject();
		...
		Long[] userIds = new Long[playerNum];
		...
		if (roomType == null) {
			...
			// 设置用户信息
			p.setPosition(HallFunctions.getWind(userIds,userId));// 设置庄家位置
...
//			userIds[p.getPosition() - 1] = p.getUserId();//在getWind里设置了
...
//			userInfos.put("playStatus", String.valueOf(Cnst.PLAYER_STATE_IN));
	...
	public synchronized static void interface_500005(WSClient channel, Map<String, Object> readData) throws Exception {
		...
		Integer needMoney = Cnst.getClubRoomNeedMoney(room.getPlayerNum(), room.getCircleNum());
		...
		// 设置用户信息
		p.setRoomId(roomId);
		p.setPosition(HallFunctions.getWind(userIds,userId));
		if (p.getPosition().equals(Cnst.WIND_EAST)) {
			room.setZhuangId(userId);
		}
		...
		//规则
		info.put("playType", String.valueOf(room.getPlayType()));
		info.put("playerNum", String.valueOf(room.getPlayerNum()));
		info.put("sanQiSuanJia", String.valueOf(room.getSanQiSuanJia()));
		info.put("danDiaoSuanJia",String.valueOf(room.getDanDiaoSuanJia()));
		info.put("gangScore", String.valueOf(room.getGangScore()));
		info.put("scoreType", String.valueOf(room.getScoreType()));
		info.put("qiangGangHu", String.valueOf(room.getQiangGangHu()));
		if(room.getPlayType().equals(Cnst.PLAY_TYPE_DAIBAO)){
			info.put("hongZhongBao", String.valueOf(room.getHongZhongBao()));
			info.put("guaDaFeng", String.valueOf(room.getGuaDaFeng()));
			info.put("buJiaBuHu", String.valueOf(room.getBuJiaBuHu()));
		}
